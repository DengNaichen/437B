...
from Functions import free_energy_hgo
import torch


def train_free_energy_two_inputs(net, x, inputs, rho, lambd, scheduler, simpson_matrix,
                                 optimizer, hgo_excluded_area, epochs, diagram=True):
    """
    ...
    for i in range(epochs):
        yhat = net(inputs)
        loss = free_energy_hgo(yhat, rho, x, simpson_matrix, hgo_excluded_area, lambd)
